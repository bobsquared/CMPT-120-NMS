...
def binaryList(lis): #Convert the list of remaining rocks on each planet to binary and print the result
...
def toBase10(lis): #Convert the binary number from the previous function to base 10, using 2s compliment
    num = 0
    negative = False
    if(lis[0] == 1):
        negative = True
...
def addOne(lis): #Add one to the binary list to finish 2s compliment
    i = len(lis) - 1
    done = False
    while(not done):
        if(lis[i] == 0):
...
def drawPlanet(lis): #Draws individual planets
...
def drawSpace(): #Draws the space between the planets
    t.penup()
    t.forward(70)
    t.pendown()

def drawAllPlanets(lis): #Draw all of the planets
    t.speed(0)
    startPos(len(lis))
    for i in range(len(lis)):
        drawPlanet(lis[i])
...
def drawAstro(planets, pos, fuel): #Draw the player
    if(endgame == 0):
        t.penup()
        t.fillcolor(determineShipColour(fuel))
        t.backward(70*(planets-pos)) #Go to the correct position
        t.right(90)
        t.forward(150)
        t.left(90)
        t.forward(25)
...
def startPos(n): #Figure out where to start drawing based on the number of planets for...
    t.penup()
    space = 0
    if((n%2)==0): #...an even number of planets
        space = n/2
        t.backward((space*50)+((space-1)*20)+10)
    else: #...an odd number of planets
        space = n//2
...
def determinePlanetFillColour(lis): #Figure out what colour the planet should be based on the number of rock specimens
    colour = (0, 0, 0)
    value = lis[2] * 17
    if(value < 1):
        colour = (0, 0, 0) #Black if there are no rocks
    elif(value < 255): #Dark to medium green if there are less than 15 rocks
        colour = (0, value, 0)
    elif(value < 455): #Light green if there are more than 15 rocks
        colour = ((value - 255), 255, (value - 255))
    else:
        colour = (200, 255, 200) #Cap the colour change
    return colour

def determinePlanetOutlineColour(lis): #Outline the planet in orange if it has fuel available, black otherwise
    colour = ""
    if(lis[1] > 0):
        colour = "orange"
    else:
...
def determineShipColour(fuel): #Colour the ship green if it has more than 10 fuel, yellow if it has less, and red if it's out of fuel
    colour = ""
    if(fuel < 1):
        colour = "red"
    elif(fuel < 10):
...
def clearBoard(): #Clear the board between drawings
    t.reset()
